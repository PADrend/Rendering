...
PipelineHandle::Ref PipelineCache::requestGraphicsHandle(const PipelineRef& pipeline, const PipelineRef& parent) {
	if(!pipeline || pipeline->getType() != PipelineType::Graphics)
		return nullptr;
	auto gPipeline = dynamic_cast<GraphicsPipeline*>(pipeline.get());

	auto shader = gPipeline->getShader();
	if(!shader || !shader->init())
		return nullptr;

	auto fbo = gPipeline->getFBO();
	if(!fbo || !fbo->validate())
		return nullptr;
	
	const auto& state = gPipeline->getState();
	size_t hash = 0;
	...
	// Create new pipeline
	vk::Device vkDevice(device->getApiHandle());
	vk::PipelineCache vkCache(handle);
	...
	for(auto& a : bs.getAttachments())
		attachments.emplace_back(convertColorBlendAttachmentState(a));
	...
	// TODO: ensure/check that there are as many blend attachments as fbo attachments
	...
	if(parent && parent->isValid())
		info.basePipelineHandle = parent->getApiHandle();

	// Create pipeline
	auto handle = new PipelineHandle(vkDevice.createGraphicsPipeline(vkCache, info), vkDevice);
	cache.emplace(hash, handle);
	return handle;
}

//---------------

PipelineHandle::Ref PipelineCache::requestComputeHandle(const PipelineRef& pipeline, const PipelineRef& parent) {
	if(!pipeline || pipeline->getType() != PipelineType::Compute)
		return nullptr;
	auto cPipeline = dynamic_cast<ComputePipeline*>(pipeline.get());

	auto shader = cPipeline->getShader();
	if(!shader || !shader->init())
		return nullptr;
	
	if(cPipeline->getEntryPoint().empty())
		return nullptr;

	size_t hash = 0;
	Util::hash_combine(hash, shader->getLayoutHash());
	Util::hash_combine(hash, cPipeline->getEntryPoint());
	...
	info.stage.pName = cPipeline->getEntryPoint().c_str();
	...
	// Create pipeline
	auto handle = new PipelineHandle(vkDevice.createComputePipeline(vkCache, info), vkDevice);
	cache.emplace(hash, handle);
	return handle;
}
