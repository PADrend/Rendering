...
public:
	...
	RENDERINGAPI static Ref create(const DeviceRef& device, BufferAllocator::Ref allocator=nullptr);

	//! Create a buffer object from an existing buffer.
	RENDERINGAPI static Ref create(const BufferStorageRef& buffer, size_t size=0, size_t offset=0, BufferAllocator::Ref allocator=nullptr);
	 ...
	 */
	RENDERINGAPI bool allocate(size_t size, ResourceUsage usage = ResourceUsage::General, MemoryUsage access = MemoryUsage::CpuToGpu, bool persistent=false);
	 ...
	 * and unbind the buffer object.
	 */
	RENDERINGAPI void upload(const uint8_t* data, size_t numBytes, size_t offset = 0);
	 ...
	 * and unbind the buffer object.
	 */
	RENDERINGAPI std::vector<uint8_t> download(size_t range, size_t offset=0);
	...
	RENDERINGAPI BufferHandle getApiHandle() const;
	// @}
